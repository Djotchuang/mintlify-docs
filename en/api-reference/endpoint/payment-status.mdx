...
<Note>
For a given transaction ID, you can make at most **6 requests per minute**. Exceeding this limit returns a **429** response.

It is generally better to rely on [webhook notifications](/en/api-reference/endpoint/webhook) to know when a payment status changes, rather than polling this endpoint.
</Note>
