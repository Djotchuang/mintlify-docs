...
<Note>
  Les transactions de paiement direct ne peuvent et ne s'expirent pas, sauf si
  vous les expirez manuellement en utilisant l'endpoint
  [expire-transaction](/fr/api-reference/endpoint/expire-transaction). Par
  conséquent, leur état final est soit **SUCCESSFUL** soit **FAILED**.
</Note>

<Warning>
  {" "}
  Le paiement direct est désactivé par défaut dans l’environnement Live. Suivez les
  étapes du guide [Activer le paiement direct sur votre API Fapshi en mode Live](https://www.fapshi.com/fr/aide-et-support/how-to-activate-direct-pay-for-your-fapshi-api)
  pour activer le paiement direct en mode live.
</Warning>
<Danger>
